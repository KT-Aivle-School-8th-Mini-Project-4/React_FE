...
import { X, BookOpen, User as UserIcon, Calendar, Hash, Tag, Star, MessageSquare, Send, Edit2, Trash2, Check, XCircle, Package, Plus, Minus, ShoppingCart, Heart } from 'lucide-react';
...
export function BookDetailDialog({ book, currentUser, purchases, onClose, onUpdateBook, onPurchaseBook, onCancelPurchase, onAddToCart, onToggleWishlist, onLikeReview, onReportReview, onToggleReviewHidden, isInWishlist, isDarkMode }: BookDetailDialogProps) {
  ...
  const handleCancelPurchase = () => {
    if (cancellablePurchases.length > 0 && onCancelPurchase && cancelQuantity > 0) {
      if (cancelQuantity > cancellablePurchases.length) {
        alert(`취소할 수 있는 수량을 초과했습니다. (취소 가능 수량: ${cancellablePurchases.length}권)`);
        return;
      }
      
      if (confirm(`${cancelQuantity}권의 구매를 취소하시겠습니까?`)) {
        const sortedPurchases = cancellablePurchases.sort((a, b) => 
                ...
                <div className="space-y-3">
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    ...
                    <div className="flex items-center justify-center gap-3 p-3 bg-red-50 rounded-lg">
                      ...
                      <button
                        onClick={() => setCancelQuantity(Math.min(cancellablePurchases.length, cancelQuantity + 1))}
                        disabled={cancelQuantity >= cancellablePurchases.length}
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    ...
                    <button
                      ...
                      disabled={cancellablePurchases.length === 0}
                      className={`w-full py-3 rounded-lg transition-colors flex items-center justify-center gap-2 ${
                        cancellablePurchases.length > 0
                  ...
                  {/* Second Row: Add to Cart & Wishlist */}
                  <div className="grid grid-cols-2 gap-4">
                    {/* Add to Cart Button */}
                    {onAddToCart && (
                      <button

...
백엔드 API 명세서에 정의된 **19개 엔드포인트**가 모두 구현되어 있습니다.
...
| API 이름 | Method | Endpoint | 파일 | 상태 |
|---------|--------|----------|------|------|
| 도서 전체 조회 | GET | `/book` | bookService.ts | ✅ 구현완료 |
...
```typescript
import { USER_ENDPOINTS, BOOK_ENDPOINTS, COMMENT_ENDPOINTS, ORDER_ENDPOINTS } from './apiEndpoints';
...
```
/services/
├── apiEndpoints.ts      # 모든 API 엔드포인트 상수 정의 (19개)

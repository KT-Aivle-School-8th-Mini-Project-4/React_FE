...
const realOrderService = {
  ...
  createOrder: async (data: CreateOrderRequest): Promise<OrderResponse> => {
    ...
    return await apiClient.post<OrderResponse>(ORDER_ENDPOINTS.CREATE_ORDER, data);
  },
  ...
  cancelOrder: async (orderId: string): Promise<OrderResponse> => {
    ...
    return await apiClient.post<OrderResponse>(ORDER_ENDPOINTS.CANCEL_ORDER(orderId));
  },

  // ✅ 주문 상세 조회 - GET /order/{orderId}
  getOrder: async (orderId: string): Promise<OrderResponse> => {
    // 🔌 외부 API 호출 - 백엔드에서 주문 상세 정보 조회
    return await apiClient.get<OrderResponse>(ORDER_ENDPOINTS.GET_ORDER(orderId));
  }
};

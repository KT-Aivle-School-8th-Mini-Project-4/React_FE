...
export default function App() {
        ...
        if (savedBooks) {
            try {
                ...
                return parsed.map((book) => ({
                    ...
                    ratings: book.ratings?.map((r) => ({ ...r, timestamp: new Date(r.timestamp) })) || [],
                    reviews: book.reviews?.map((r) => ({ ...r, timestamp: new Date(r.timestamp) })) || [],
                    stock: book.stock || 0
                }));
            } catch (e) {
    ...
    const handleBulkDelete = () => {
        ...
        if (confirm(`선택한 ${selectedBookIds.length}권의 도서를 삭제하시겠습니까?`)) {
            ...
            const deleteRecords = deletedBooks.map(book => ({
    ...
    const handleRestoreBook = (deleteRecord) => {

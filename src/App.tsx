...
export interface Purchase {
  ...
  userId: string;
  purchaseDate: Date;
  status?: 'pending' | 'shipped' | 'delivered'; // Purchase status
}
...
export default function App() {
  ...
  const handlePurchaseBook = (bookId: string, quantity: number, silent: boolean = false): boolean => {
    ...
    // Create purchase records with 'shipped' status (cannot be cancelled)
    ...
    for (let i = 0; i < quantity; i++) {
      newPurchases.push({
        ...
        status: 'shipped' // Automatically set to shipped (cannot cancel)
      });
    }
  ...
  const handleCancelPurchase = (purchaseIds: string[]) => {
    ...
    // Check if any purchase has been shipped or delivered
    const purchasesToCancel = purchases.filter(p => purchaseIds.includes(p.id));
    const shippedPurchases = purchasesToCancel.filter(p => p.status === 'shipped' || p.status === 'delivered');
    
    if (shippedPurchases.length > 0) {
      alert('배송이 시작된 구매는 취소할 수 없습니다.');
      return;
    }
  ...
  const handleCartCheckout = () => {
    ...
    // Create purchases for all cart items with 'shipped' status
    const newPurchases: Purchase[] = [];
    ...
    cartItems.forEach(item => {
      for (let i = 0; i < item.quantity; i++) {
        newPurchases.push({
          id: `${Date.now()}_${item.book.id}_${i}`,
          bookId: item.book.id,
          userId: currentUser.id,
          purchaseDate: new Date(),
          status: 'shipped' // Automatically set to shipped (cannot cancel)
    ...
    alert(`${totalQuantity}권의 도서 구매가 완료되었습니다.`);
  ...
  const handleRemoveFromWishlistBulk = (bookIds: string[]) => {
    ...
    const wishlist = currentUser.wishlist || [];
    const updatedWishlist = wishlist.filter(id => !bookIds.includes(id));
    ...
    <div className={`min-h-screen transition-colors duration-300 ${isDarkMode ? 'bg-gray-900' : 'bg-gray-50'}`}>
                  ...
                  {isAdmin && (
                    <div className="relative group">
                      <button
                        ...
                        className="w-10 h-10 flex items-center justify-center bg-purple-600 text-white rounded-lg hover:bg-purple-700 transition-all duration-300 shadow-sm hover:scale-110"
                      >
      ...
      {selectedBook && currentUser && (
        <BookDetailDialog
          ...
          isInWishlist={currentUser.wishlist?.includes(selectedBook.id)}
          isDarkMode={isDarkMode}
        />
      )}
